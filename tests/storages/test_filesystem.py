...
class TestFileSystemStorage(object):
    ...
    def test_constructor_sets_directory(self, storage, tmpdir):
        assert storage.directory == str(tmpdir)

    def test_relative_directory_is_normalized(self, storage):
        storage.directory = 'foobar'
        assert storage.directory == os.path.abspath('foobar')

    def test_delete_removes_the_file(self, storage, tmpdir):
    ...
    def test_exists_returns_false_if_file_doesnt_exist(self, storage):
        assert storage.exists('foobar') is False

    def test_exists_returns_true_if_file_exists(self, storage, tmpdir):
    ...
    def test_size_returns_file_size_in_bytes(self, storage, tmpdir):
